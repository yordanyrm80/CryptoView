...
class TrackerProvider extends ChangeNotifier {
  ...
  /// Automatically matches all open buys and sells using FIFO (First In, First Out)
  Future<int> autoMatchFIFO({String? symbol, String? exchange}) async {
    final cleanSymbol = symbol?.replaceAll('-', '/').toUpperCase();
    final cleanExchange = exchange?.toLowerCase();

    // 1. Get all buys and sells eligible for matching
    List<TransactionModel> availableBuys = _transactions.where((tx) {
      if (tx.type != 'buy') return false;
      if (cleanSymbol != null && tx.symbol.replaceAll('-', '/').toUpperCase() != cleanSymbol) return false;
      if (cleanExchange != null && tx.exchange.toLowerCase() != cleanExchange) return false;
      return getRemainingAmount(tx) > 0.000001;
    }).toList();

    List<TransactionModel> availableSells = _transactions.where((tx) {
    ...
    availableBuys.sort((a, b) => a.date.compareTo(b.date));
    availableSells.sort((a, b) => a.date.compareTo(b.date));

    int matchesCreated = 0;
    int buyIdx = 0;
    int sellIdx = 0;

    // Track remaining amounts in memory during loop
    final Map<int, double> remainingMap = {};
    for (var b in availableBuys) {
      remainingMap[b.id!] = getRemainingAmount(b);
    }
    for (var s in availableSells) {
      remainingMap[s.id!] = getRemainingAmount(s);
    }

    while (buyIdx < availableBuys.length && sellIdx < availableSells.length) {
      final buy = availableBuys[buyIdx];
      final sell = availableSells[sellIdx];

      final remBuy = remainingMap[buy.id!] ?? 0.0;
      final remSell = remainingMap[sell.id!] ?? 0.0;
      ...
      final amountToMatch = remBuy < remSell ? remBuy : remSell;
      ...
      final newRemBuy = remBuy - amountToMatch;
      final newRemSell = remSell - amountToMatch;

      remainingMap[buy.id!] = newRemBuy;
      remainingMap[sell.id!] = newRemSell;

      if (newRemBuy < 0.00001) {
        await DatabaseHelper.instance.updateTransaction(
          buy.copyWith(isMatched: true).toMap(),
        );
        buyIdx++;
      }
      if (newRemSell < 0.00001) {
        await DatabaseHelper.instance.updateTransaction(
          sell.copyWith(isMatched: true).toMap(),
        );
        sellIdx++;
